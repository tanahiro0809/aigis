...
gulp.task(`sass`, function() {
  ...
  .pipe(autoprefixer({
    overrideBrowserslist: browserVersion,
    cascade: false
  }))
  .pipe(postcss([cssMqpacker]))
...
gulp.task(`html-hint`, function() {
  return gulp.src([`${DIST_DIR}/**/*.html`, `!${DIST_DIR}/**/ssi/**/*.html`, `!${DIST_DIR}/**/_styleguide/**/*.html`])
  .pipe(htmlhint())
...
gulp.task(`watch`, function(done){
  gulp.watch(`${SRC_DIR}/**/*.scss`, gulp.task([`sass`]));
  gulp.watch(`${DIST_DIR}/**/*.html`, gulp.task([`html-hint`]));
  done();
});
...
gulp.task('textValid', function(callback) {
  ...
  .pipe(
    intercept(function(file){
      interceptCnt++;
      var relativePath = file.path.replace(new RegExp(`^${__dirname}/${DIST_DIR}/`),'/');
...
/*w3cValid*/
gulp.task('w3cValid',function(done){
  ...
  pathArr.map((thisPath) => {
    var relativePath = path.resolve(thisPath).replace(new RegExp(`^${__dirname}/${DIST_DIR}/`),'/');
    ...
    if (error) {
      ...
      JSON.parse(error.toString().split('\n')[1]).messages
        .forEach((json) => {
          ...
          url = path.resolve(url.replace('file:', '')).replace(new RegExp(`^${__dirname.normalize()}/${DIST_DIR}/`), '/');
...
/*textValid w3cValidを直列実行するタスク*/
gulp.task('valid',
  gulp.series('textValid','w3cValid')
);